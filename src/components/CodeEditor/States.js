...
const StatesCode = ({ value, children }) => {
  const [list, setList] = useState(
    new Map(
      Object.keys(value).map(key => {
        return [key, { id: key, code: variableToString(value[key]?.initValue) }];
      })
    )
  );
  ...
  return children({
    ...
    value: transform(
      ...
      (result, value) => {
        const { id, code } = value;
        result[id] = {
          initValue: JSON.parse(code),
          setStateName: camel(`set-${id}`, { caseStyle: 'lower' })
        };
      },
      {}
    ),
    ...
    onCodeChange: code => {
      try {
        ...
        const func = new Function(`${code} return initStateValue;`);
...
const States = createWithRemoteLoader({
  modules: [
      ...
      onClick={() => {
        modal({
          ...
          withDecorator: render => {
            return <StatesCode value={value}>{props => render(props)}</StatesCode>;
          },
              ...
              <Button
                ...
                onClick={() => {
                  const addModalApi = formModal({
                    ...
                    children: <FormInfo column={1} list={[<Input name="name" label="状态名" rule="REQ LEN-0-100" />]} />
                  });
                }}
              >
                添加状态
              ...
              {(() => {
                ...
                return (
                  <MonacoEditor
                    ...
                    defaultValue={`const initStateValue = ${item.code || '""'};`}
                    loading="正在加载代码编辑器..."
